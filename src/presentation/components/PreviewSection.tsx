...
import type { Essay, TourProps } from '../../domain/types';
import TourTooltip from './TourTooltip';

interface PreviewSectionProps {
  essay: Essay;
  totalWordCount: number;
  setShowExaminer: (show: boolean) => void;
  ...
  return (
    <div className="bg-[#f4f1ea] border-l-2 border-stone-900 h-full flex flex-col overflow-hidden relative">
      <div className="p-6 flex justify-between items-center z-10 border-b-2 border-stone-900 bg-white">
        ...
        <span
          className={`text-[10px] font-bold px-2 py-1 uppercase border border-stone-900 ${
            totalWordCount > 300
          ...
          <div className="font-serif leading-loose text-stone-900 text-lg whitespace-pre-wrap selection:bg-yellow-300 selection:text-stone-900 column-count-1">
            {generateFullEssay() ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-stone-300 gap-4 select-none border-2 border-dashed border-stone-300 p-8">
                ...
                <span className="font-serif italic text-xl text-center">
                  The page is blank.
                  <br />
      ...
      <div className="p-0 z-20 flex border-t-2 border-stone-900 relative">
        ...
        >
          {!isPaid && <>{examinerUsageCount < 1 ? <Type size={14} /> : <Lock size={14} />}</>}
          {isPaid
            ? 'Get Feedback'
            : examinerUsageCount < 1
              ? totalWordCount < 30
                ? 'Free Examiner (Min 30 Words)'
                : 'Free Examiner'
              : 'Unlock Examiner'}
        </button>
        <button
