import { BookOpen, Check, Crown, Flame, X, Zap } from 'lucide-react';
...
const PricingModal: React.FC<PricingModalProps> = ({
  ...
  const getPlanTier = (name: string | null) => {
      ...
      <div className="bg-[#f4f1ea] border-2 border-stone-900 shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] max-w-6xl w-full relative max-h-[95vh] flex flex-col">
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 p-4 md:p-8">
          <div className="text-center mb-10 pt-4 md:pt-0">
            <span className="inline-block px-3 py-1 bg-stone-900 text-white text-[10px] font-black uppercase tracking-widest mb-4">
              Invest in your band score
            </span>
            <h2 className="text-4xl font-serif font-black text-stone-900">Choose Your Plan</h2>
            {activePlan && (
              <p className="text-stone-500 mt-2 font-mono text-sm italic">
                You currently have the{' '}
                <span className="text-stone-900 font-bold">{activePlan}</span> active.
              </p>
            )}
            {!activePlan && (
              <p className="text-stone-500 mt-2 font-mono text-sm italic">
                Affordable AI tutoring for the Nepali student.
              </p>
            )}
          </div>
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      {plan.originalPrice && (
                        <p className="text-sm text-stone-400 line-through mb-1 font-bold opacity-80">
                          {plan.originalPrice}
                        </p>
                      )}
                      <p className="text-3xl font-black text-stone-900 leading-none">
                        {plan.price === 'FREE' ? (
                          ...
                          <>
                            <span className="text-sm font-bold align-top mr-0.5 select-none opacity-60">
                              Rs
                            </span>
                      ...
                      <div className="flex flex-col items-end mt-1">
                        ...
                        {plan.dailyPrice && (
                          <p className="text-xs text-green-600 font-black uppercase tracking-tight mt-1 italic">
                            {plan.dailyPrice}
                          </p>
                        )}
                      </div>
                  ...
                  <ul className="space-y-3 mb-8 flex-1">
                    {plan.features.map((feat) => (
                      <li
                        key={feat}
                        className="flex items-center gap-2 text-[11px] font-bold text-stone-700"
                      >
                  ...
                  <button
                    ...
                    className={`w-full py-4 font-black uppercase tracking-widest transition-all border-2 border-stone-900 ${
                      isActive || isLower || (plan.id === 'free' && !!activePlan)
          ...
          {!activePlan && !isLoggedIn && (
            ...
            >
              <p className="text-sm font-bold text-stone-900 mb-2 uppercase tracking-tighter italic">
                Already Paid?
              </p>
              <p className="text-[10px] text-stone-500 mb-6 font-medium uppercase tracking-widest px-4">
                Your access is now tied to your account. Please log in to restore your premium
                features.
              </p>
