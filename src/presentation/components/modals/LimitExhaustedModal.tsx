import { Award, Rocket, X, Zap } from 'lucide-react';
import type React from 'react';

interface LimitExhaustedModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpgrade: (plan: string | null) => void;
}

const LimitExhaustedModal: React.FC<LimitExhaustedModalProps> = ({
  isOpen,
  onClose,
  onUpgrade,
}) => {
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/90 z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-[#f4f1ea] border-4 border-stone-900 shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] max-w-md w-full relative overflow-hidden">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-stone-500 hover:text-stone-900 z-20"
        >
          <X size={24} />
        </button>
        ...
        <div className="p-8 pt-10 text-center relative z-10">
          ...
          <p className="text-stone-700 font-medium text-lg leading-relaxed mb-8">
            Don't lose your flow. Get 24-hour unlimited access for the price of only{' '}
            <span className="text-stone-900 font-black border-b-4 border-yellow-400">Rs. 50</span>.
          </p>
