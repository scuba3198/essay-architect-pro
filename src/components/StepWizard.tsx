...
import { Essay, TourProps, Topic, EssaySectionKey } from '../types';
...
const StepWizard: React.FC<StepWizardProps> = ({ currentStep, setCurrentStep, essay, handleInputChange, tourProps, topic, isPaid, onLimitReached, freeUsageCount, onIncrementUsage }) => {
    const steps = [
    ...
    const [refineModal, setRefineModal] = useState<{ isOpen: boolean; text: string; section: EssaySectionKey | ''; field: string }>({ isOpen: false, text: '', section: '', field: '' });
    ...
    const handleAutocomplete = async (section: EssaySectionKey, field: string, currentText: string) => {
        const canUseFree = freeUsageCount < 10;
        if (!isPaid && !canUseFree) {
            onLimitReached();
            return;
