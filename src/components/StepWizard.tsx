...
import type { Essay, EssaySectionKey, Topic, TourProps } from "../types";
...
const VocabularyPill: React.FC<VocabularyPillProps> = ({ words, onSelect }) => {
  return (
    <div className="flex flex-wrap gap-3 mt-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      {words.map((word) => (
        ...
        >
          <Sparkles
            size={10}
            className="opacity-0 group-hover:opacity-100 transition-opacity absolute left-1"
          />
...
const StepWizard: React.FC<StepWizardProps> = ({
  ...
  const steps = [
    ...
    },
    {
      ...
      subtitle: "Point, Explain, Evidence",
      icon: "II",
    },
    {
      id: "body2",
      title: "The Second Argument",
      subtitle: "Reinforce your Stance",
      icon: "III",
    },
    {
      id: "conclusion",
  ...
  const [refineModal, setRefineModal] = useState<{
    ...
    field: string;
  }>({ isOpen: false, text: "", section: "", field: "" });
  ...
  const openRefine = (section: EssaySectionKey, field: string, text: string) => {
  ...
  const handleRefineAccept = (newText: string) => {
    if (refineModal.section !== "") {
      handleInputChange(refineModal.section as EssaySectionKey, refineModal.field, newText);
    }
  };
  ...
  const RefineButton: React.FC<RefineButtonProps> = ({ section, field, text }) => {
    const canUseFree = freeUsageCount < 10;
    ...
    return (
      <div className="flex gap-2">
        <button
          ...
          title={isPaid || canUseFree ? "Autocomplete sentence" : "Unlock for AI Autocomplete"}
        >
          {generatingField === `${section}-${field}` ? (
          ...
          {isPaid || canUseFree ? "Complete" : "Unlock Pro"}
        </button>
        <button
          type="button"
          ...
          title={isPaid || canUseFree ? "Polished by AI" : "Unlock for AI Refinement"}
        >
    ...
    <div className="bg-white border-2 border-stone-900 shadow-[8px_8px_0px_0px_rgba(231,229,228,1)] flex flex-col h-full relative">
      ...
      <div className="border-b-2 border-stone-900 p-6 relative bg-[#f4f1ea]">
        <div className="flex justify-between items-end mb-6">
          <div>
            <span className="text-xs font-black uppercase tracking-widest text-stone-500 mb-1 block">
        ...
        <div className="flex items-center w-full h-1 bg-stone-300">
          {steps.map((step, idx) => (
            <div
              key={step.id}
              className={`h-full flex-1 transition-all duration-300 ${idx <= currentStep ? "bg-stone-900" : "bg-transparent"}`}
            ></div>
          ))}
        </div>
      </div>
          ...
          <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
            <div className="bg-stone-50 p-6 border-l-4 border-stone-900">
              <div className="flex gap-3">
                ...
                <div className="text-sm font-serif text-stone-800 italic leading-relaxed">
                  Your goal is simple: Introduce the topic and state your position clearly. Never
                  copy the question directly!
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end border-b-2 border-stone-900 pb-2 mb-4">
                <div className="flex flex-col text-lg font-bold font-serif text-stone-900">
                  <span>1. Paraphrase the Question</span>
                  <span className="text-[10px] font-sans font-normal text-stone-500 uppercase tracking-widest mt-1">
                    Required • Write 1 Sentence
                  </span>
                </div>
          ...
          <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end border-b-2 border-stone-900 pb-2">
                <div>
                  <div className="block text-lg font-bold font-serif text-stone-900">
                    1. Topic Sentence
                  </div>
                  <p className="text-xs font-mono text-stone-500 uppercase">
                    Write 1 Sentence • The Main Idea
                  </p>
                </div>
                <RefineButton
                  section={`body${currentStep}` as EssaySectionKey}
                  field="topicSentence"
                  text={
                    (
                      essay[`body${currentStep}` as EssaySectionKey] as unknown as Record<
                        string,
                        string
                      >
                    ).topicSentence || ""
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleInputChange(
                    `body${currentStep}` as EssaySectionKey,
                    "topicSentence",
                    e.target.value,
                  )
                }
              ...
              <VocabularyPill
                words={
                  currentStep === 1
                    ? ["Firstly,", "To begin with,", "The primary reason is"]
                    : ["Secondly,", "Furthermore,", "In addition,"]
                }
                onSelect={(word) =>
                  handleVocabularyInsert(`body${currentStep}`, "topicSentence", word)
                }
              />
            </div>
            ...
            <div className="space-y-4 pt-4">
              <div className="flex justify-between items-end border-b-2 border-stone-900 pb-2">
                <div>
                  ...
                  <p className="text-xs font-mono text-stone-500 uppercase">
                    Write 1 Sentence • Expand on the point
                  </p>
                </div>
                <RefineButton
                  section={`body${currentStep}` as EssaySectionKey}
              ...
              <VocabularyPill
                ...
                onSelect={(word) =>
                  handleVocabularyInsert(`body${currentStep}`, "explanation", word)
                }
              />
            </div>
            ...
            <div className="space-y-4 pt-4">
              ...
              <VocabularyPill
                ...
                onSelect={(word) =>
                  handleVocabularyInsert(`body${currentStep}`, "concluding", word)
                }
              />
            </div>
          </div>
            ...
            <div className="bg-stone-50 p-6 border-l-4 border-stone-900">
              <div className="flex gap-3">
                ...
                <div className="text-sm font-serif text-stone-800 italic leading-relaxed">
                  ...
                  <span className="bg-stone-900 text-white px-1">DO NOT</span> introduce new
                  arguments here.
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end border-b-2 border-stone-900 pb-2">
                <div>
                  ...
                  <p className="text-xs font-mono text-stone-500 uppercase">
                    Write 1 Sentence • The Recap
                  </p>
                </div>
                <RefineButton
                  section="conclusion"
                  field="summary"
                  text={essay.conclusion.summary}
                />
              </div>
              <textarea
            ...
            <div className="space-y-4 pt-4">
              <div className="flex justify-between items-end border-b-2 border-stone-900 pb-2">
                <div>
                  ...
                  <p className="text-xs font-mono text-stone-500 uppercase">
                    Write 1 Sentence • Look to the future
                  </p>
                </div>
                <RefineButton
                  section="conclusion"
                  field="finalThought"
                  text={essay.conclusion.finalThought}
                />
              </div>
              <textarea
